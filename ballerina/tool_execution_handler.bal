...
isolated class ToolExecutionHandler {
    ...
    isolated function init(ToolRegistry toolRegistry) {
    ...
    isolated function executeTool(FunctionCall toolCall, string executionId, string sessionId, Context ctx)
        ...
        observe:ExecuteToolSpan span = setupToolSpan(toolName, self.toolRegistry, toolCall);

        ToolOutput|ToolExecutionError|LlmInvalidGenerationError output = self.toolRegistry.execute(toolCall, ctx);
