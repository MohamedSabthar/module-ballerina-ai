...
# + pendingApprovals - Present only when the turn paused for human approval. Lists the tool
#                       call(s) awaiting a decision; reply with a `ChatApprovalMessage` keyed by
#                       these `ApprovalRequest.id`s (via the `chat/approve` resource) to resume the run.
...
# Sent to a chat service's `chat/approve` resource to resume the run for `sessionId`.
#
...
public type ApprovableChatService distinct service object {
    *ChatService;
    resource function post chat/approve(@http:Payload ChatApprovalMessage request) returns ChatRespMessage|error;
};
