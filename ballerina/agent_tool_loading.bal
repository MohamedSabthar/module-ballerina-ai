...
#
# + toolRegistry - The tool store containing registered tools
...
isolated function getFilteredTools(ToolRegistry toolRegistry, ToolLoadingStrategy strategy,
        ChatMessage[] messages, ModelProvider model) returns ChatCompletionFunctions[] {
    ChatCompletionFunctions[] registeredTools = from Tool tool in toolRegistry.tools.toArray()
        select {
            name: tool.name,
