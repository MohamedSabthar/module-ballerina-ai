...
isolated function setupToolSpan(string toolName, ToolRegistry toolRegistry, FunctionCall toolCall) returns observe:ExecuteToolSpan {
    observe:ExecuteToolSpan span = observe:createExecuteToolSpan(toolName);
    string? toolCallId = toolCall.id;
    if toolCallId is string {
        span.addId(toolCallId);
    }
    string? toolDescription = toolRegistry.getToolDescription(toolName);
    if toolDescription is string {
        span.addDescription(toolDescription);
    }
    span.addType(toolRegistry.isMcpTool(toolName) ? observe:EXTENTION : observe:FUNCTION);
